...
DownloadManager::DownloadManager(const QString& mangaName, const QDir& scansDirectory, QObject* parent) :
    QObject(parent),
...
QString DownloadManager::saveFileName(void) {
    ...
    if (directories.size() > 2) {
        ...
        pathName += "Misc/";
        emit message("Warning: file will be downloaded in a /home/valentin/Images/Scans/Misc/", Warning);
    }
...
DownloadHTMLManager::DownloadHTMLManager(const QString& mangaName, QDir scansDirectory, QObject* parent) :
    DownloadManager(mangaName, scansDirectory, parent),
    _htmlPages() {
}
