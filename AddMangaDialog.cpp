...
//    addMangaLayout->setAlignment(Qt::AlignLeft);
    ...
    _messageListModel = new MessageListModel;
...
void AddMangaDialog::checkMangaName(void) {
    ...
    if (htmlPages.size() == 0) {
        ...
        if (!htmlPage.contains("href=\"/Manga-Scan/"+_mangaNameLineEdit->text())) {
            ...
            QDir scansDir("/home/valentin/Images/Scans");
            if (!scansDir.mkdir(_mangaNameLineEdit->text())) {
...
void AddMangaDialog::downloadPreviewImage(void) {
    QDir scansDirectory("/home/valentin/Images/Scans");
    scansDirectory.mkdir("Manga-Scan");
    scansDirectory.cd("Manga-Scan");
    scansDirectory.mkdir(_mangaNameLineEdit->text());
...
void AddMangaDialog::finish(void) {
    QDir scansDirectory("/home/valentin/Images/Scans");
    scansDirectory.rename("/home/valentin/Images/Scans/Manga-Scan/"+_mangaNameLineEdit->text()+"/cover.jpg",
                     "/home/valentin/Images/Scans/"+_mangaNameLineEdit->text()+"/cover.png");
    scansDirectory.rmdir("Manga-Scan/"+_mangaNameLineEdit->text());
    scansDirectory.rmdir("Manga-Scan");

    if (addManga()) {
        updateMessage(_mangaNameLineEdit->text()+" has been successfuly added to manga list.", DownloadManager::Success);
    } else {
        scansDirectory.rmdir(_mangaNameLineEdit->text());
    }
}
...
bool AddMangaDialog::addManga() {
    ...
    QFile dbTxtFile(scansDir.path()+"/"+_mangaNameLineEdit->text()+"/db.txt");
    ...
    QFile mangaInfoTxtFile(scansDir.path()+"/"+_mangaNameLineEdit->text()+"/mangaInfo.txt");
